...
// in that case, compute the convex hull of v, and see if both have the same
// area
{
  int i, j, k;
  int c1 = 0;
...
// 1 if point is on the segment; 0 if not on the line; -1 if on the line but not
// on the segment
{
    ...
    if (j == 0) {
      ...
      for (i = 0; i < n; i++)
        if (pv[i].x * pv[i + 1].x < -EPS) {
          y = pv[i + 1].y -
              pv[i + 1].x * (pv[i].y - pv[i + 1].y) / (pv[i].x - pv[i + 1].x);
          if (y > 0)
            j++;
        }
      return (j % 2);
...
void show(PT &p) { cout << "(" << p.x << ", " << p.y << ")" << endl; }
...
int PInterP(vector<PT> &p1, vector<PT> &p2, vector<PT> &p3) {
  ...
  if (m > 1 && n > 1)
    for (i = 0; i < m; i++)
      for (j = 0; j < n; j++)
        if (intersection(p1[i], p1[(i + 1) % m], p2[j], p2[(j + 1) % n], pp) ==
            0) {
